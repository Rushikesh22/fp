    ...
    #pragma omp parallel
    {
        ...
        for (std::size_t l = 0; l < warmup; ++l)
        {
            ...
            else
            {
                ...
                {
                    fp_triangular_matrix_vector(transpose, alpha, a_compressed[thread_id][k], x[k_offset + k], beta, y[k_offset + k], symmetric);
                }
            }
        }
        ...
        for (std::size_t l = 0; l < measurement; ++l)
        {
            if (use_blas)
            {
                ...
                for (std::size_t k = 0; k < a.size(); ++k)
                {
                    time_accumulated += blas_triangular_matrix_vector(transpose, n, alpha, a_packed[k], x[k], beta, y[k], symmetric);
                }
            }
            else
            {
                for (std::size_t k = 0; k < a_compressed[thread_id].size(); ++k)
                {
                    time_accumulated += fp_triangular_matrix_vector(transpose, alpha, a_compressed[thread_id][k], x[k_offset + k], beta, y[k_offset + k], symmetric);
                }
            }
        }
...
#if defined(BENCHMARK)
    // output some metrics
    const double gflops = measurement * a.size() * (symmetric ? 2 : 1) * (n * n) / (time / omp_get_max_threads()) * 1.0E-9;
